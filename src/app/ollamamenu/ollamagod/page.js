...
const Page = () => {
  ...
  const [mots,setMots] = useState(50)
    ...
    if (input.trim() !== '') {
      try {
        ...
        if(preset==0){
          ...
          `adopte le role de Déeese Gova  , maitre spirituel  ,  
          réponds  aux questions avec ${mots} mots maximum par question , 
          tu es  très calme , très spirituel ,  généreuse et adopte un language de compassion.      
  ...
  const handleSynthesize = async (text) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8010/synthesize', {
        ...
        ref_speaker_wav: "speakers/helenaspiritual2.wav",
        options: {
          temperature: 0.75,
          length_penalty: 1,
  ...
  return (
    <>
      ...
      {audioSrc && (
        <audio controls autoPlay key={audioSrc} ref={audioRef}>
